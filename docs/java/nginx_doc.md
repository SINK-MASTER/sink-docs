...
举个具体的例子，晚高峰乘坐地铁的时候，入站口经常会有地铁工作人员大喇叭“请走 `B` 口， `B` 口人少车空....”，这个工作人员的作用就是负载均衡。![图片](https://mmbiz.qpic.cn/mmbiz_png/wic3gC6OynIHq4ib8cUQHHib4XYgOvwlD3kwHuAO9nPTCicEYk4wyqsWTWB5baCyRylByb4mWa0oprCia0CXUP0WRdg/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)
...
并不是每个浏览器都支持 `gzip` 的，如何知道客户端是否支持 `gzip` 呢，请求头中的 `Accept-Encoding` 来标识对压缩的支持。![图片](https://mmbiz.qpic.cn/mmbiz_png/wic3gC6OynIHq4ib8cUQHHib4XYgOvwlD3kfibWf8R3dPWDZc217BBhqYxmTOklmIoAFmREialfl2xSDrVR3H0eYSZQ/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)启用 `gzip` 同时需要客户端和服务端的支持，如果客户端支持 `gzip` 的解析，那么只要服务端能够返回 `gzip` 的文件就可以启用 `gzip` 了,我们可以通过 `Nginx` 的配置来让服务端支持 `gzip` 。下面的 `respone` 中 `content-encoding:gzip` ，指服务端开启了 `gzip` 的压缩方式。![图片](https://mmbiz.qpic.cn/mmbiz_png/wic3gC6OynIHq4ib8cUQHHib4XYgOvwlD3kk941XKS4ZK2Xre0rI6G2aVRyfXK7cNWcicGUV2USYaO31mT0KFfZicNQ/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)在 `/etc/nginx/conf.d/` 文件夹中新建配置文件 `gzip.conf` ：
...
`Nginx` 的内部结构是由核心部分和一系列的功能模块所组成。这样划分是为了使得每个模块的功能相对简单，便于开发，同时也便于对系统进行功能扩展。`Nginx` 的模块是互相独立的,低耦合高内聚。![图片](https://mmbiz.qpic.cn/mmbiz_png/wic3gC6OynIHq4ib8cUQHHib4XYgOvwlD3kjfogsTfItWwYLrBLTLOehyLibh7GibMicoSFLiaO4oCM8yQUibNRyvB6A7w/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)
